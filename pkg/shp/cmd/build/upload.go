...
type UploadCommand struct {
	...
	method uploadMethod // upload method to be used, either stream or bundle, defaults to stream
...
func (u *UploadCommand) Run(p *params.Params, ioStreams *genericclioptions.IOStreams) error {
	...
	case bundling:
		...
		u.pw.WithOnPodModifiedFn(func(pod *corev1.Pod) error {
			switch pod.Status.Phase {
			case corev1.PodFailed:
				u.stop()
				return fmt.Errorf("build pod '%s' has failed", pod.GetName())
